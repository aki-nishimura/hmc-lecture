...
\frame{
	...
	\item Having covered details, we now state the algorithm more precisely.
		\begin{itemize}
		\item \textit{Mass matrix} $\Mass$ does not need to be proportional to the identity.
...
\frame{
	...
	\item Performance of HMC depends critically on the choice of
		...
		\arraycolsep=2pt
			...
			\end{array}
		\end{itemizedEquation*}
	\item $\integrationTime$ determines HMC's mixing rate and is often best randomized.
	\item Choice of $\stepsize$ presents a trade-off between the acceptance rate and the cost of proposal generation.
		\begin{itemize}
		\item $\stepsize$ should be small enough for a reasonable acceptance rate but no smaller.
		\end{itemize}
	\item $\Mass$ can be used to effectively \textit{precondition} the target distribution, or linearly transform the parameter $\bposition$.
	\end{itemize} 
}
...
\frame{
	...
	\item But let's say we aim for $\Mass = \text{Var}(\bposition)$ as commonly done.
		\begin{itemize}
		\item $\bSigma$ can be estimated during the ``warm-up \!/\! burn-in'' period.
		\item Empirical covariance is unstable in high-dimensions; we may estimate the marginal variances only and set $\Mass = \textrm{diag}(\bm{\widehat{\Sigma}})$.
		...
		\item We want to keep  the narrowest ``width'' of the distribution of $\Mass^{-1/2} \bposition$ constant, so scale $\Mass$ to have the largest eigenvalue $1$.
		\end{itemize}
	\end{itemize}
}
...
\frame{
	\frametitle{Tuning integrator stepsize}
	\begin{itemize}
...
\frame{
	...
	\begin{figure}
		...
		\caption{
			...
			The stability limit of the integrator is $\bposition$-dependent and is approximately $\exp(\position_1)$.
