...
\subsection{With exact Hamiltonian dynamics}

\frame{
	\frametitle{HMC on Gaussian targets}
	\begin{itemize}
	\item As explained later, HMC's performance on multivariate Gaussians can be understood via its behavior on a $1$-d Gaussian with mass $m = 1$.
	\item So we study HMC's behavior for $\position \sim \normal(0, \sigma^2)$ and $\momentum \sim \normal(0, 1)$, or
		\begin{itemizedEquation*}
...
\subsection{With leapfrog integrator dynamics}
...
\frame{
	...
	\item For $\stepsize < 2 \sigma$, we have the following formula for $\solutionOp_{\stepsize}^n$:
		...
		\end{bmatrix}
			...
			\end{bmatrix}
		\end{itemizedEquation*}
		where $\phi = \cos^{-1}\!\left( 1 - \frac{\stepsize^2}{2 \sigma^2} \right)$.
	\end{itemize}
}
...
\frame{
	...
	\item Acceptance probability is given as $\min\{1, \exp(-\Delta H)\}$ where 
		...
		\Delta H 
			= - \log \frac{
					\pi(\bposition_{L \stepsize},  \bmomentum_{L \stepsize})
				}{
	...
	\centering
		...
		\vspace{-\baselineskip}
		\caption{Average Hamiltonian error $\mathbb{E} \left[ \Delta H \right]$ as a function of $\stepsize$ \\ when $\integrationTime = 2 \pi \sigma$ and $L = \lceil \integrationTime / \stepsize \rceil$.}
		\end{minipage}
	\end{figure}
}

% TODO: discuss a multivariate case.



\section{Tuning HMC: theory and practice}

\subsection{Theoretical guidance on tuning HMC for optimal performance}

\subsection{Tuning HMC in real-life and common ``gotchas''}
