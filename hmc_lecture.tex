...
\frame{
	...
	\begin{block}{HMC transition rule}
		Given the current state $(\bposition_0, \bmomentum_0)$, HMC generates the next state as follows:
		\begin{enumerate}
			\item Randomize momentum: $\bmomentum_0 \sim \mathcal{N}\left(\mathbf{0}, \textcolor{lava}{\Mass} \right)$.
			\item Generate a proposal $(\bposition^*, \bmomentum^*) \approx (\bposition(\integrationTime), \textcolor{lava}{\pm} \bmomentum(\integrationTime))$ \textcolor{lava}{by using the leapfrog integrator} to approximate the solution of Hamilton's equation
				\vspace{-.5\baselineskip}
				\begin{equation}
				\vspace{-.5\baselineskip}
				\begin{aligned}
				\frac{\diff \bposition}{\diff t} 
				&= \textcolor{lava}{\Mass^{-1}} \bmomentum, \quad
				\frac{\diff \bmomentum}{\diff t} 
				= - \nabla_{\bposition} U(\bposition) 
				\end{aligned}
				\end{equation}
				with initial condition $(\bposition(0), \bmomentum(0)) = (\bposition_0, \bmomentum_0)$.
			\item Accept or reject the proposal \textcolor{lava}{with the acceptance probability
				\begin{itemizedEquation*}
...
\frame{
	...
	\begin{block}{HMC transition rule}
		Given the current state $(\bposition_0, \bmomentum_0)$, HMC generates the next state as follows:
		\begin{enumerate}
			\item Randomize momentum: \textcolor{lava}{$\bmomentum_0 \given \bposition_0 \sim \pi_{\Momentum \given \Position}(\, \cdot \given \bposition_0)$}.
			\item Generate a proposal $(\bposition^*, \bmomentum^*) \approx (\bposition(\integrationTime),  \bmomentum(\integrationTime))$ by using \textcolor{lava}{a reversible and volume-preserving integrator} to solve Hamilton's equation
				\vspace{-.5\baselineskip}
				\begin{equation}
				\vspace{-.5\baselineskip}
				\begin{aligned}
